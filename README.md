...
Multi-tenant, multi-source RAG service (FastAPI · Vertex AI · Qdrant) that unifies owner-uploaded Google review exports and on-chain Solana reviews behind a pluggable `ReviewSource` interface — with per-business `business_id` payload filtering for tenant isolation, an LLM-driven query parser that extracts structured metadata filters from natural-language questions, and a recall@k eval harness measuring retrieval quality across a 556-review corpus: a flat k=50 recovers only 17% of relevant docs on analytical queries vs 97% at k=500, validating the query-type-aware dynamic-k design; LLM-extracted rating filters decouple complaint queries from k-starvation entirely (100% recall by k=50). Part of the [Gromopo](https://github.com/gromopo-tech/gromopo) system.
...
Retrieval k scales with query intent: analytical summaries use k=1000 (near-exhaustive), comparison queries k=100, specific-example queries k=30. The recall@k eval harness validates this design: on a 556-review corpus, a flat k=50 recovers only 17% of relevant docs for broad analytical queries vs 97% at k=500 — a fixed low k would silently miss the majority of relevant context.
...
| **Eval** | `python3 eval/run_eval.py` — k-sweep recall@k harness (objective relevant sets, no hand-labeling); run before and after embedding/prompt changes to catch retrieval regressions |
